            ...
            $.confirm({
                ...
                buttons: {
                    Да: function () {
                        ...
                        app.mainAjax("/mark/save-mark.html", {
                            ...
                            $.confirm({
                                ...
                                buttons: {
                                    Ok: function () {
                                        $('html, body').animate({ scrollTop: $('#pjax_mark').offset().top-40 }, 500);
                                        $('#btn_pjax_mark').click();
                                    }
                                }
                            });
            ...
            $.confirm({
                ...
                buttons: {
                    Да: function () {
                        ...
                        app.mainAjax("/mark/move-mark.html", {
                            selector: JSON.stringify(selector)
                        }, true).then(function(res) {
                            console.log(res);
                            switch (res) {
...
$('#pjax_mark').on('change', '.mark_name', function() {
    var id = $(this).data('id');
    var name = $(this).val();
    $.ajax({
        url: '/mark/rename.html',
        data: {
            id: id,
            name: name
        },
...
$('#pjax_mark').on('click', '.mark_del', function() {
    var id = $(this).data('id');
    ...
    $.confirm({
        ...
        buttons: {
            Да: {
                btnClass: 'btn-green',
                action: function () {
                    app.mainAjax("/mark/del-mark.html", {
                        id: id
                    }).then(function(res) {
                        console.log(res);
                        $('#btn_pjax_mark').click();
...
$('#pjax_mark').on('click', '.mark_move', function() {
    var id = $(this).data('id');
    mark_move_id = id;
    var marks = document.getElementsByClassName('mark_move');
    for (var i = 0; i < marks.length; i++) {
        var current_id = $(marks[i]).data('id');
        if ( current_id === id ) $(document.getElementsByClassName('mark_watch')[i]).click();
    }
...
// просмотр подмены
$('#pjax_mark').on('click', '.mark_watch', function() {
    try {
